...
add_action('admin_enqueue_scripts', 'my_style_shepacom');
function my_style_shepacom()
{
    $dir = site_url().'/wp-content/plugins/shepacom-vipfiledownload/css/style.css';

    wp_register_style('custom_shepacom_css', $dir);
    wp_enqueue_style('custom_shepacom_css');
}
...
shepacomFileDownload::init();

class shepacomFileDownload
{
    ...
    public static function init()
    {
        ...
        register_activation_hook($dir . 'shepacom_file_download.php', array(__CLASS__, 'install'));
...
', '', 'yes');
        ...
        add_option("vip_shepacom_api", "YOUR-API-KEY", '', 'yes');
        add_option("vip_shepacom_return_url", get_option("siteurl"), '', 'yes');
        date_default_timezone_set("Asia/Tehran");
        $inDate = date("Y-m-d");
    ...
    public static function admin_init()
    {
        ...
        register_setting('vip_options', 'vip_shepacom_api');
        register_setting('vip_options', 'vip_shepacom_return_url');
    ...
    {
        add_menu_page("فروشگاه", "فروشگاه", 'manage_options', 'shepacom-file-download', array(__CLASS__, 'admin_dashboard'), 'https://cdn.shepa.com/plugins/shepacom-vipfiledownload/img/basket.png');
        add_submenu_page('shepacom-file-download', "محصولات", "محصولات", 'manage_options', "shepacom-file-download-products", array(__CLASS__, 'admin_products_router'));
        add_submenu_page('shepacom-file-download', "تنظیمات", "تنظيمات", 'manage_options', "paypal-file-download-settings", array(__CLASS__, 'admin_settings'));
        add_submenu_page('shepacom-file-download', "فروش ها", "فروش ها", 'manage_options', "paypal-file-download-transactions", array(__CLASS__, 'admin_transactions'));
        add_menu_page("اشتراک VIP", "اشتراک VIP", 'manage_options', 'shepacom-vip', array(__CLASS__, 'admin_dashboard'), 'https://cdn.shepa.com/plugins/shepacom-vipfiledownload/img/vip.png');
        add_submenu_page('shepacom-vip', "اشتراک ها", "اشتراک ها", 'manage_options', "shepacom-vip-accounts", array(__CLASS__, 'admin_vip_router'));

        add_submenu_page('shepacom-vip', "تنظیمات", "تنظيمات", 'manage_options', "shepacom-vip-settings", array(__CLASS__, 'admin_settingsvip'));
        add_submenu_page('shepacom-vip', "کاربران VIP", "کاربران VIP", 'manage_options', "shepacom-vip-orders", array(__CLASS__, 'admin_orders'));
    }
    ...
    protected static function admin_products_edit()
    {
		...
		<a href="' . get_option('siteurl') . '/wp-admin/admin.php?page=shepacom-file-download-products">&laquo; بازگشت به صفحه محصولات</a>
		<form action="' . get_option('siteurl') . '/wp-admin/admin.php?page=shepacom-file-download-products&action=edit&id=' . $_GET['id'] . '" method="post">
    ...
    protected static function admin_products_delete()
    {
		...
		window.location="' . get_option('siteurl') . '/wp-admin/admin.php?page=shepacom-file-download-products"';
        echo '//--></script>';
    }
	...
	<div class="wrap">
        ...
        if (count($products) == 0) {
            ...
            foreach ($products as $product) {
                ...
                echo '<tr class="alternate author-self status-publish iedit" valign="top">
					...
					<td class="post-title column-title"><strong><a class="row-title" href="' . get_option('siteurl') . '/wp-admin/admin.php?page=shepacom-file-download-products&action=edit&id=' . $product['id'] . '">' . $product['name'] . '</a></strong></td>
					<td class="">' . $product['cost'] . ' ' . get_symbol_of_currency($product['currency']) . '</td>
					<td class="" style="text-align:center;">' . $product['downloads'] . '</td>
					<td class="" style="text-align:center;"><a href="' . get_option('siteurl') . '/wp-admin/admin.php?page=shepacom-file-download-products&action=edit&id=' . $product['id'] . '">ويرايش</a></td>
					<td class="" style="text-align:center;"><a href="' . get_option('siteurl') . '/wp-admin/admin.php?page=shepacom-file-download-products&action=delete&id=' . $product['id'] . '" onClick="if(confirm(\'آيا از حذف اين مورد اطمينان داريد؟ !\')) { return true;} else { return false;}">حذف</a></td>
				</tr>';
            }
        }
        echo '</tbody>
		...
		<form action="' . get_option('siteurl') . '/wp-admin/admin.php?page=shepacom-file-download-products&action=add" method="post">
		<table class="form-table">
			<tr valign="top">
				<th scope="row">نام محصول</th>
    ...
    public static function admin_settings()
    {
        ...
        echo '<tr valign="top">
					...
					<td><input type="text" name="paypal_return_url" style="width:250px;" value="' . get_option('paypal_return_url') . '" /><br />لينک بازگشت به سايت شما پس از انجام تراکنش در درگاه shepa.com</td>
				</tr>';
        echo '<tr valign="top">
    ...
    {
        $more = '<a href="#TB_inline?width=350&inlineId=paypal_file_download_form" class="thickbox" title="قرارد دادن لينک پرداخت shepa.com"><img src="https://cdn.shepa.com/plugins/shepacom-vipfiledownload/img/basket.png" alt="قرارد دادن لينک پرداخت shepa.com" /></a>';
        return $context . $more;
    }

    public static function vip_media_button($context)
    {
        $more = '<a href="#TB_inline?width=350&inlineId=vip_linkdownload_form" class="thickbox" title="قرارد دادن لينک پرداخت shepa.com و لینک دانلود VIP"><img src="https://cdn.shepa.com/plugins/shepacom-vipfiledownload/img/vip.png" alt="vip" /></a>';
        return $context . $more;
    }

    public static function vip_data_button($context)
    {
        $more = '<a href="#TB_inline?width=350&inlineId=vip_data_form" class="thickbox" title="قرار دادن شورتکد محتوای VIP"><img src="https://cdn.shepa.com/plugins/shepacom-vipfiledownload/img/vipdata.png" alt="قرار دادن شورتکد محتوای VIP" /></a>';
        return $context . $more;
    }
    ...
    public static function add_pfd_form()
    {
		...
		function insert_pfd_button(){
            ...
            construct = '<form name=\"frm_shepacom' + product_id + '\" action=\"" . get_option('siteurl') . "/?checkout=' + product_id + '\" method=\"post\"><input type=\"image\" name=\"submit\" src=\"' + image + '\" value=\"1\"></form>';";
        echo "var wdw = window.dialogArguments || opener || parent || top;
			wdw.send_to_editor(construct);
		}";
        echo "function insert_pfd_link(){
			product_id = jQuery('#product_selector').val()
            construct = '<form name=\"frm_shepacom' + product_id + '\" action=\"" . get_option('siteurl') . "/?checkout=' + product_id + '\" method=\"post\"><input type=\"image\" src=\"\" value=\"' + image + '\"></form>';";
        echo "var wdw = window.dialogArguments || opener || parent || top;
			wdw.send_to_editor(construct);
		}
	</script>";
		...
		<div class="wrap" style="text-align:right;direction:rtl;">
			<div>	
				<div style="padding:15px 15px 0 15px;">
					<h3 style="font-size:16pt"><br />قرارد دادن لينک پرداخت shepa.com</h3>
        ...
        echo '</td>
						...
						<tr>
							...
							<td><input type="text" id="button_image_url" value="https://cdn.shepa.com/plugins/shepacom-vipfiledownload/img/paynow.png" style="width:220px;" /></td>
						</tr>
					</table>
				</div>';
        echo '<div style="padding:15px;">
    ...
    protected static function ipn()
    {
        ...
        $price = isset($_SESSION['shepacom_amount']) ? $_SESSION['shepacom_amount'] : 0;
        ...
        if (isset($_GET['status']) && isset($_GET['token'])) {
            ...
            $factor_number = isset($_GET['factorNumber']) ? $_GET['factorNumber'] : null;
            $message = isset($_GET['message']) ? $_GET['message'] : null;

            if (isset($status) && $status == 1) {

                $api_key = get_option('vip_shepacom_api');
                ...
                $url_payment = "https://merchant.shepa.com/api/v1/verify";
                if ($api_key == "sandbox") $url_payment = "https://sandbox.shepa.com/api/v1/verify";
                $result = self::common($url_payment, $params);
    ...
    protected static function get_email()
    {
        ...
        echo '<html>
				<link rel="stylesheet"  media="all" type="text/css" href="'.site_url().'/wp-content/plugins/shepacom-vipfiledownload/css/style.css">
				<body class="vipbody">	
				<div class="mrbox2" > 
        ...
        <label class="title"> تصویر امنیتی</label>
          ...
          <input type="submit" name="submit" id="submit" value="پرداخت" class="dlbtn"/>
          <br />
        </form>
		</div>
		</body>
    ...
    public static function var_listener()
    {
        ...
        if (get_query_var("checkout") == NULL) {
            ...
            if (isset($_POST['submit'])) {
                ...
                $_SESSION['shepacom_amount'] = $amount;
                ...
                if (extension_loaded('curl')) {
                    ...
                    $params = array(
                        ...
                        'api' => $api_key,
                        'amount' => $amount,
                        'callback' => $redirect."&factorNumber=".$resnum,
                        'description' => $desc
                    );
                    if (!empty($_mobile)) $params['mobile'] = $_mobile;
                    $url_payment = "https://merchant.shepa.com/api/v1/token";
                    if ($api_key == "sandbox") $url_payment = "https://sandbox.shepa.com/api/v1/token";
                    $result = self::common($url_payment, $params);
                    ...
                    if (!empty($result->success)) {
                        ...
                        echo '<form action="' . $result->result->url . '" method="get" name="frmPay"><noscript><input type="submit" value="Pay" /></noscript></form><script>document.frmPay.submit();</script>';
                        ...
                        $html = '<html><head><link media="all" rel="stylesheet" type="text/css" href="'.site_url().'/wp-content/plugins/shepacom-vipfiledownload/css/style.css"></head><body class="vipbody"><div class="mrbox2">';
  ...
  <a href="' . get_option('siteurl') . '/wp-admin/admin.php?page=shepacom-vip-accounts">&laquo; بازگشت به صفحه اشتراک ها</a>
  <form action="' . get_option('siteurl') . '/wp-admin/admin.php?page=shepacom-vip-accounts&action=edit&id=' . $_GET['id'] . '" method="post">
    <table class="form-table">
      <tr valign="top">
    ...
    protected static function admin_vip_account_delete()
    {
        ...
        echo '<script type="text/javascript"><!--
		window.location="' . get_option('siteurl') . '/wp-admin/admin.php?page=shepacom-vip-accounts"';
        echo '//--></script>';
    }
  ...
  <table class="widefat post fixed" cellspacing="0">
    ...
    <tbody>';
        ...
        <td>
		<a class="row-title" href="' . get_option('siteurl') . '/wp-admin/admin.php?page=shepacom-vip-accounts&action=edit&id=' . $account['id'] . '">' . $account['name'] . '</a></td>';
                echo '<td>' . $account['descript'] . '</td>
       <td>' . $account['cost'] . ' ' . get_symbol_of_currency($account["currency"]) . '</td>
       <td>' . $account['day'] . '</td>
       <td>' . $account['per_day'] . '</td>
       <td><a class="row-title" href="' . get_option('siteurl') . '/wp-admin/admin.php?page=shepacom-vip-accounts&action=delete&id=' . $account['id'] . '">حذف</a></td>
      </tr>';
            }
        }
        echo '</tbody>
  ...
  <form action="' . get_option('siteurl') . '/wp-admin/admin.php?page=shepacom-vip-accounts&action=add" method="post">
    <table class="form-table">
      <tr valign="top">
        <th scope="row">نام اشتراک</th>
  ...
  <h3>تنظيمات</h3>';
      ...
      <tr valign="top">
        ...
        <td><input type="text" name="vip_shepacom_api" style="width:300px;" value="' . get_option('vip_shepacom_api') . '" />
          <br />
          کلید API شما در سايت shepa.com</td>
      </tr>

      <tr valign="top">
        <th scope="row">آدرس بازگشتي</th>
        <td><input type="text" name="vip_shepacom_return_url" style="width:250px;" value="' . get_option('vip_shepacom_return_url') . '" />
          <br />
          لينک بازگشت به سايت شما پس از انجام تراکنش در درگاه shepa.com</td>
      </tr>';
        ...
        if ((isset($_GET['status']) && $_GET['status'] == 'success') && isset($_GET['token'])) {
            ...
            if ((isset($_GET['status']) && $_GET['status'] == 'success')) {
                ...
                if (!empty($result->success)) {
                    ...
                    $card_number = isset($_POST['card_pan']) ? $_POST['card_pan'] : null;


                    if ($price == $result->result->amount) {
            ...
            echo "<div align='center' dir='rtl' style='font-family:tahoma;font-size:11px;border:1px dotted #c3c3c3; width:60%; line-height:20px;margin-left:20%'>تراکنش شما <font color='red'><b>لغو گردید</b></font>.<br/><p align='right' style='margin-right:15px'>تراکنش لغو گردید<br/> لطفا به صفحه اصلی سایت برگشته و مجددا خرید خود را انجام دهید.</p><a href='", get_option('siteurl'), "'>بازگشت به صفحه اصلي</a><br/><br/></div>";
        }
    }
        ...
        if (get_query_var("buy_account_vip") == NULL) {
            if (get_query_var("vipdownload") == NULL) {
                ...
                if (get_exp_day($Exp_Vip, true)) {
                    $html = '<html><head><link rel="stylesheet"  media="all" type="text/css" href="'.site_url().'/wp-content/plugins/shepacom-vipfiledownload/css/style.css"></head><body class="vipbody" >';
                    ...
                    if (($Extant_Daily == -1 or ($Extant_Daily >= 1)) and ($Extant_Daily <= $Exp_Per_Day)) {
                        ...
                        $html .= '<hr>	<b> همین الان محصول را با پرداخت آنلاین بخرید </b><br><br>
								<form name="frm_shepacom' . $product['id'] . '" action="' . $SiteURL . '" method="get">
  ...
  <div class="wrap" style="text-align:right; direction:rtl;">
    <div>
        ...
        <div style="padding:15px 15px 0 15px;">
			<h3 style="font-size:16pt">قرارد دادن لينک پرداخت shepa.com و نمایش لینک دانلود کاربران VIP</h3>
			<span>لطفا محصول مورد نظرتان را از لينک زير انتخاب نماييد</span>
		</div>
